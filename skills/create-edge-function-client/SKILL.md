...
description: Generate a typed client for calling a Supabase Edge Function's HTTP API from its OpenAPI spec — request/response types plus a thin fetch wrapper. Use whenever the user wants to call an existing Edge Function from a frontend, from another Edge Function (server-to-server), or from a separate external codebase, and wants generated types instead of hand-written fetch calls. Trigger on "generate a client for this function," "add types for calling this API," "I need to call function X from function Y," or "consume this OpenAPI spec" — and just as much on plain-language phrasing that never says "client," "types," or "OpenAPI" at all, e.g. "connect to the partner API," "make our app talk to the partner service," "get data from the third-party integration," "pull orders from the partner's API."
---
